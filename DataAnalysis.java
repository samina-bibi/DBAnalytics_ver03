...
public class DataAnalysis {
	...
	*/    
	public static int getMaxOccurrence(Connection conn, int dbLength) throws SQLException {
		...
		String request = requestElement(conn, "SELECT COUNT(write_time) AS `max_occurrence` FROM NormalThings WHERE write_time>0.045"); //0.015
	...
	/**
	* Queries DB to see if write time increases with number of writes by calculating the moving average of the write time.
	* @param conn [Connection] to enable the establishing online connection to database
	...
	*/    
	public static Double getWTIncrease(Connection conn) throws SQLException {
		...
		String request = requestElement(conn, "select sum((x - x_bar) * (y - y_bar)) / sum((x - x_bar) * (x - x_bar)) as Slope "
	...
	*/    
	...
	Double variance =0D;
		...
		try {
			Statement statement = conn.createStatement();
			ResultSet results = statement.executeQuery("SELECT (SUM([write_time]*[write_time])-(SUM([write_time])*SUM([write_time]))"
